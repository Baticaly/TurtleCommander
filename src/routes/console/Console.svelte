<script>
	...
	supabase
		...
		.on(
			'postgres_changes',
			{ event: 'UPDATE', schema: 'public', table: 'turtles', filter: `id=eq.${$turtleConsole.id}` },
			(payload) => {
...
<div class="w-screen h-screen bg-consoleBackground">
	...
	<div class="absolute bottom-10 left-10">
		<div class="bg-neutral-500 w-64 h-64 grid grid-cols-4 grid-rows-4 border-2 border-neutral-900">
			{#each turtleInfo.inventory as itemSlot}
				<div class="h-full w-full border-2 border-neutral-900" />
			{/each}
		</div>
	</div>
	<div class="absolute bottom-0 w-screen flex justify-center">
		<div class="h-20 bg-consoleText flex justify-center items-center px-3 rounded-t-xl">
